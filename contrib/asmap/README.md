...
be used in Smithcoin Core with the `-asmap` option.
...
to binary before being used with Smithcoin Core.
...
the `smithcoin-cli getnodeaddresses` command.
...
```
smithcoin-cli getnodeaddresses 0 > addrs.json
```
