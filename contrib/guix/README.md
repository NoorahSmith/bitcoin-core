# Bootstrappable Smithcoin Core Builds

This directory contains the files necessary to perform bootstrappable Smithcoin
Core builds.
...
To build Smithcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Smithcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/smithcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/smithcoin-core/guix.sigs).
