#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Smithcoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing smithcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='smithcoin-cli', help='smithcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Smithcoin address to which the faucet should send')
...
parser.add_argument('smithcoin_cli_args', nargs='*', help='Arguments to pass on to smithcoin-cli (default: -signet)')

args = parser.parse_args()

if args.smithcoin_cli_args == []:
    args.smithcoin_cli_args = ['-signet']


def smithcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.smithcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = smithcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = smithcoin_cli(['getnewaddress', 'faucet', 'bech32'])
