...
"""Script for verifying Smithcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from smithcoincore.org and smithcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/smithcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://smithcoincore.org"
HOST2 = "https://smithcoin.org"
VERSIONPREFIX = "smithcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Smithcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"smithcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by smithcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the smithcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes smithcoin.org lags behind smithcoincore.org.)')
    )
