...
Provides utilities for managing copyright headers of `The Smithcoin Core
...
---------------------------------------------------------
Updates all the copyright headers of `The Smithcoin Core developers` which were
changed in a year more recent than is listed. For example:
```
// Copyright (c) <firstYear>-<lastYear> The Smithcoin Core developers
```
will be updated to:
```
// Copyright (c) <firstYear>-<lastModifiedYear> The Smithcoin Core developers
```
...
those cases:
```
// Copyright (c) <year> The Smithcoin Core developers
```
will be updated to:
```
// Copyright (c) <year>-<lastModifiedYear> The Smithcoin Core developers
```
where the update is appropriate.

copyright\_header.py insert \<file\>
------------------------------------
Inserts a copyright header for `The Smithcoin Core developers` at the top of the
...
If the file already has a copyright for `The Smithcoin Core developers`, the
script will exit.
...
gen-smithcoin-conf.sh
===================

Generates a smithcoin.conf file in `share/examples/` by parsing the output from `smithcoind --help`. This script is run during the
release process to include a smithcoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-smithcoin-conf.sh
```
