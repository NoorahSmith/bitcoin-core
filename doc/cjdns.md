# CJDNS support in Smithcoin Core

It is possible to run Smithcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Smithcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Smithcoin network and individual nodes.
...
before it will work with your Smithcoin Core node. This is described in steps
...
## Run Smithcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Smithcoin Core
...
When enabled, this option tells Smithcoin Core that it is running in an
...
IPv6 local network. This helps Smithcoin Core perform better address management:
...
CJDNS support was added to Smithcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `smithcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/smithcoin/smithcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Smithcoin Core

There are several ways to see your CJDNS address in Smithcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `smithcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `smithcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `smithcoin-cli help getnodeaddresses` for details.
