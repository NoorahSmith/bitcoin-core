...
This guide describes how to build smithcoind, command-line utilities, and GUI on NetBSD.
...
It is not necessary to build wallet functionality to run smithcoind or the GUI.

###### Descriptor Wallet Support

`sqlite3` is required to enable support for [descriptor wallets](https://github.com/smithcoin/smithcoin/blob/master/doc/descriptors.md).
...
Smithcoin Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkgin zeromq
```
...
There are many ways to configure Smithcoin Core. Here is an example that
