Smithcoin Core version 0.18.0 is now available from:

  <https://smithcoincore.org/bin/smithcoin-core-0.18.0/>
...
`/Applications/Smithcoin-Qt` (on Mac) or `smithcoind`/`smithcoin-qt` (on
Linux).
...
recommended to use Smithcoin Core on unsupported systems.

Smithcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Smithcoin Core does not yet change appearance when
...
`contrib/init/smithcoind.service`, it has been changed to use
`/var/lib/smithcoind` as the data directory instead of
`~smithcoin/.smithcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/smithcoind` will
exist has enough space (check using `df -h /var/lib/smithcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/smithcoin/smithcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:11001:11001` (this is an extra `:11001` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://smithcoincore.org/en/contact/) the Smithcoin Core
...
The systemd init file (`contrib/init/smithcoind.service`) has been changed
to use `/var/lib/smithcoind` as the data directory instead of
`~smithcoin/.smithcoin`. This change makes Smithcoin Core more consistent with
...
`datadir` option in `/etc/smithcoin/smithcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/smithcoin/smithcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/smithcoin/smithcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/smithcoin/smithcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/smithcoin/smithcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/smithcoin/smithcoin/blob/master/doc/smithcoin-conf.md)
  about the `smithcoin.conf` file describes how to use it to configure
  Smithcoin Core.

- A new document introduces Smithcoin Core's BIP174 [Partially-Signed
  Smithcoin Transactions
  (PSBT)](https://github.com/smithcoin/smithcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/smithcoin/smithcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Smithcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Smithcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/smithcoin/smithcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/smithcoin/smithcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Smithcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Smithcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/smithcoin/smithcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/smithcoin/smithcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart smithcoind with the `-deprecatedrpc=generate`
...
- A new `smithcoin-wallet` tool is now distributed alongside Smithcoin
...
This section describes planned changes to Smithcoin Core that may affect
other Smithcoin software and services.

- Since version 0.16.0, Smithcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Smithcoin Core 0.20 (expected about
  a year after 0.18), Smithcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Smithcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Smithcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Smithcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Smithcoin Core
...
- A sub-project of Smithcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Smithcoin Core.  See their [project
  page](https://github.com/smithcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Smithcoin Core's own implementation, although entropy
  gathered by Smithcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Smithcoin Core a
...
- On macOS, Smithcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `smithcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libsmithcoin_server.a` (jnewbery)
...
- #13926 [Tools] smithcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like smithcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `smithcoin-qt -version` output formatting (hebasto)
...
- #11625 Add SmithcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libsmithcoin_server` (laanwj)
...
- #15154 Configure: smithcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/smithcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Smithcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libsmithcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in smithcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Smithcoin Core (Christewart)
...
- #12246 Bugfix: Only run smithcoin-tx tests when smithcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_smithcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_smithcoin` (MarcoFalke)
...
- #14186 smithcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_smithcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update smithcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/smithcoin-conf.md (hebasto)
...
- #15354 Add missing `smithcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct smithcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support smithcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/smithcoin/).
