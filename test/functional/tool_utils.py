...
# Copyright 2016-present The Smithcoin Core developers
...
class ToolUtils(SmithcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_smithcoin_tx()
        self.skip_if_no_smithcoin_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "smithcoin-util-test.json", encoding="utf8") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./smithcoin-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./smithcoin-tx":
