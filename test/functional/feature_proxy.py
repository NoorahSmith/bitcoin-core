...
"""Test smithcoind with different proxy configuration.

Test plan:
- Start smithcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on smithcoind side:
...
class ProxyTest(SmithcoinTestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: smithcoind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
