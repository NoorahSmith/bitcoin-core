...
#
# Be aware that smithcoind and smithcoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in smithcoind.
#
...
# C locale.
#
# In contrast, smithcoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where smithcoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of smithcoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in smithcoind but
# "1,230000" in smithcoin-qt.
#
