// Copyright (c) 2025 The Smithcoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries smithcoin-node, smithcoin-gui.
  -M, --monolithic       Run monolithic binaries smithcoind, smithcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show this help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'smithcoin-qt [ARGS]' or 'smithcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'smithcoind [ARGS]' or 'smithcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'smithcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'smithcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'smithcoin-tx [ARGS]'.
...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_smithcoin [ARGS]'.
  chainstate [ARGS] Run smithcoin kernel chainstate util, equivalent to running 'smithcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_smithcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_smithcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "smithcoin-gui" : "smithcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "smithcoin-node" : "smithcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("smithcoin-cli");
            // Since "smithcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("smithcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("smithcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_smithcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("smithcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_smithcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_smithcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("smithcoin-util");
        } else {
...
//! Execute the specified smithcoind, smithcoin-qt or other command line in `args`
...
//!             be a relative path to a smithcoind, smithcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/smithcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/smithcoin/smithcoin/pull/31375#discussion_r1861814807)
