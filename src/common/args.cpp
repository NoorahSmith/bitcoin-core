...
const char * const SMITHCOIN_CONF_FILENAME = "smithcoin.conf";
const char * const SMITHCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //smithcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", SMITHCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Smithcoin
    //   new: C:\Users\Username\AppData\Local\Smithcoin
    // macOS: ~/Library/Application Support/Smithcoin
    // Unix-like: ~/.smithcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Smithcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Smithcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Smithcoin";
#else
    // Unix-like
    return pathRet / ".smithcoin";
#endif
#endif
}
