...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<SmithcoinUnit>("SmithcoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. smithcoin_de.qm (shortcut "de" needs to be defined in smithcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. smithcoin_de_DE.qm (shortcut "de_DE" needs to be defined in smithcoin.qrc)
...
static const char* qt_argv = "smithcoin-qt";

SmithcoinApplication::SmithcoinApplication()
...
void SmithcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the SmithcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", SmithcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void SmithcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool SmithcoinApplication::createOptionsModel(bool resetSettings)
{
...
void SmithcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new SmithcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &SmithcoinGUI::quitRequested, this, &SmithcoinApplication::requestShutdown);
...
void SmithcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void SmithcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool SmithcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void SmithcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &SmithcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &SmithcoinApplication::handleRunawayException);
    connect(this, &SmithcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &SmithcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void SmithcoinApplication::parameterSetup()
{
...
void SmithcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void SmithcoinApplication::requestInitialize()
{
...
void SmithcoinApplication::requestShutdown()
{
...
void SmithcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // smithcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SmithcoinGUI::handlePaymentRequest);
            connect(window, &SmithcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void SmithcoinApplication::handleRunawayException(const QString &message)
{
...
void SmithcoinApplication::handleNonFatalException(const QString& message)
{
...
WId SmithcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool SmithcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SmithcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(smithcoin);
    Q_INIT_RESOURCE(smithcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(SMITHCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see smithcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see smithcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse smithcoin.conf, determine network, switch to network specific
    ...
    // smithcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
