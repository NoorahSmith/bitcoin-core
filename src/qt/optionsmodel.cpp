...
/** Call node.updateRwSetting() with Smithcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Smithcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/smithcoin/smithcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because smithcoin 22.x will not try to read these.
...
//! Convert enabled/size values to smithcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from smithcoin -prune setting.
...
//! Get pruning size value to show in GUI from smithcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplaySmithcoinUnit")) {
        settings.setValue("DisplaySmithcoinUnit", QVariant::fromValue(SmithcoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplaySmithcoinUnit");
    if (unit.canConvert<SmithcoinUnit>()) {
        m_display_smithcoin_unit = unit.value<SmithcoinUnit>();
    } else {
        m_display_smithcoin_unit = SmithcoinUnit::BTC;
        settings.setValue("DisplaySmithcoinUnit", QVariant::fromValue(m_display_smithcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if smithcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_smithcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<SmithcoinUnit>() == m_display_smithcoin_unit) return;
    m_display_smithcoin_unit = new_unit.value<SmithcoinUnit>();
    QSettings settings;
    settings.setValue("DisplaySmithcoinUnit", QVariant::fromValue(m_display_smithcoin_unit));
    Q_EMIT displayUnitChanged(m_display_smithcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/smithcoin/smithcoin/pull/8273
    ...
    // (https://github.com/smithcoin-core/gui/issues/567).
