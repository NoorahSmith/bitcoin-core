...
#include <smithcoin-build-config.h> // IWYU pragma: keep

#include <qt/smithcoingui.h>
...
const std::string SmithcoinGUI::DEFAULT_UIPLATFORM =
...
SmithcoinGUI::SmithcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &SmithcoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SmithcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SmithcoinGUI::showModalOverlay);
...
void SmithcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Smithcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and smithcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &SmithcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SmithcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SmithcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Smithcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Smithcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Smithcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Smithcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a smithcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Smithcoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SmithcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SmithcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SmithcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SmithcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SmithcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SmithcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SmithcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SmithcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &SmithcoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &SmithcoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &SmithcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SmithcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SmithcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SmithcoinGUI::showDebugWindow);
}

void SmithcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void SmithcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SmithcoinGUI::setCurrentWalletBySelectorIndex);
...
void SmithcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SmithcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SmithcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SmithcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SmithcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SmithcoinGUI::enableHistoryAction(bool privacy)
{
...
void SmithcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SmithcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SmithcoinGUI::removeWallet);
...
WalletController* SmithcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void SmithcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SmithcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SmithcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SmithcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SmithcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SmithcoinGUI::incomingTransaction);
    connect(this, &SmithcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void SmithcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void SmithcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SmithcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SmithcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SmithcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void SmithcoinGUI::createTrayIcon()
{
...
void SmithcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SmithcoinGUI::toggleHidden);
...
void SmithcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SmithcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SmithcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SmithcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SmithcoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SmithcoinGUI::openClicked()
{
...
void SmithcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SmithcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SmithcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SmithcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SmithcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SmithcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SmithcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SmithcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Smithcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void SmithcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SmithcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SmithcoinGUI::updateHeadersSyncProgressLabel()
{
...
void SmithcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SmithcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SmithcoinGUI::quitRequested);
...
void SmithcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SmithcoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &SmithcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void SmithcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SmithcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SmithcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SmithcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void SmithcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SmithcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SmithcoinGUI::incomingTransaction(const QString& date, SmithcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SmithcoinUnits::formatWithUnit(unit, amount, true));
...
void SmithcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SmithcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SmithcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SmithcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SmithcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SmithcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SmithcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SmithcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SmithcoinGUI::updateWindowTitle()
{
...
void SmithcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SmithcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SmithcoinGUI::detectShutdown()
{
...
void SmithcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SmithcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SmithcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SmithcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SmithcoinGUI::subscribeToCoreSignals()
{
...
void SmithcoinGUI::unsubscribeFromCoreSignals()
{
...
bool SmithcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<SmithcoinUnit> units = SmithcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SmithcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, SmithcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const SmithcoinUnit u : SmithcoinUnits::availableUnits()) {
        menu->addAction(SmithcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(SmithcoinUnit newUnits)
{
    setText(SmithcoinUnits::longName(newUnits));
}
