...
static void SetupSmithcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " smithcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The smithcoin-tx tool is used for creating and modifying smithcoin transactions.\n\n"
                "smithcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded smithcoin transaction, or with \"-create [commands]\" to create a hex-encoded smithcoin transaction.\n"
                "\n"
                "Usage: smithcoin-tx [options] <hex-tx> [commands]\n"
                "or:    smithcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded smithcoin transaction
